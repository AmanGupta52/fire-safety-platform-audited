...
function FieldChrome({ label, error, hint, required, children }: FieldWrapProps & { children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1.5">
      {label && <label className="text-xs font-medium text-slateink">{label} {required && <span className="text-brand">*</span>}</label>}
      {children}
...
const baseInputStyles =
  'w-full rounded border border-line bg-white px-3.5 py-2.5 text-sm text-ink placeholder:text-slateink/60 focus:border-ink focus:outline-none transition-colors';
...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ label, error, hint, required, className, ...props }, ref) => (
    <FieldChrome label={label} error={error} hint={hint} required={required}>
      <input ref={ref} className={clsx(baseInputStyles, error && 'border-brand', className)} {...props} />
    </FieldChrome>
  )
);
...
export const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ label, error, hint, required, className, ...props }, ref) => (
    <FieldChrome label={label} error={error} hint={hint} required={required}>
      <textarea ref={ref} className={clsx(baseInputStyles, 'min-h-[96px] resize-y', error && 'border-brand', className)} {...props} />
    </FieldChrome>
  )
);
